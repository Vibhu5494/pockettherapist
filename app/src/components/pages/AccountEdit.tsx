...
console.log(typeof state?.firstName,'state?.firstName');
    ...
    <PageContainer>
      ...
      <KeyboardAvoidingPageContent>
        <ScrollView alwaysBounceVertical={false} contentContainerStyle={styles.formContainer}>
          ...
          <View style={styles.formSection}>
            <TextField  onChangeText={(text) => onUpdate({ username: text })} label="Username*" value={state?.username} disabled={true} />
          ...
          <View style={styles.formSection}>
            ...
            <HelperText style={{top:-11}} type="error">{error&& state?.firstName===undefined ||state?.firstName?.length===0? 'Required':''}</HelperText>
          </View>
          <View style={styles.formSection}>
            <TextField onChangeText={(text) => onUpdate({ lastName: text })} label="Last Name*" value={state?.lastName} disabled={!isLoaded} />
            <HelperText style={{top:-11}} type="error">{error&& state?.lastName===undefined ||state?.lastName?.length===0? 'Required':''}</HelperText>
          </View>
          <View style={styles.formSection}>
